...
async fn test_google_provider_system_message() {
    ...
    assert_eq!(contents[0].parts[0].text, Some("Write a hello world program".to_string()));
}
...
async fn test_google_generation_config() {
    ...
    assert_eq!(gemini_msgs[0].parts[0].text, Some("Test config".to_string()));
}

#[tokio::test]
async fn test_google_provider_error_handling() {
    // 测试错误处理 - 使用无效配置
    let provider = GoogleProvider::new("invalid-key".to_string(), Some("invalid-model".to_string()));
